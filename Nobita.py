...
async def start_R4V4N():
    ...
    if ekish:
        ...
        try:
            ...
            await shi(functions.channels.JoinChannelRequest(channel="@NOBITA_SPAMMER_OP"))
            await shi(functions.channels.JoinChannelRequest(channel="@NOBITA_SPAMMER_OP"))
            botid = telethon.utils.get_peer_id(botme)
            SMEX_USERS.append(botid)
        except Exception as e:
    ...
    if baish:
        ...
        try:
            ...
            await yaa(functions.channels.JoinChannelRequest(channel="@NOBITA_SPAMMER_OP"))
            await yaa(functions.channels.JoinChannelRequest(channel="@NOBITA_SPAMMER_OP"))
            botid = telethon.utils.get_peer_id(botme)
            SMEX_USERS.append(botid)
        except Exception as e:
    ...
    if teish:
        ...
        try:
            ...
            await dav(functions.channels.JoinChannelRequest(channel="@NOBITA_SPAMMER_OP"))
            await dav(functions.channels.JoinChannelRequest(channel="@NOBITA_SPAMMER_OP"))
            botid = telethon.utils.get_peer_id(botme)
            SMEX_USERS.append(botid)
        except Exception as e:
    ...
    if tfour:
        ...
        try:
            ...
            await raj(functions.channels.JoinChannelRequest(channel="@NOBITA_SPAMMER_OP"))
            await raj(functions.channels.JoinChannelRequest(channel="@NOBITA_SPAMMER_OP"))
            botid = telethon.utils.get_peer_id(botme)
            SMEX_USERS.append(botid)
        except Exception as e:
    ...
    if tfive:
        ...
        try:
            ...
            await put(functions.channels.JoinChannelRequest(channel="@NOBITA_SPAMMER_OP"))
            await put(functions.channels.JoinChannelRequest(channel="@NOBITA_SPAMMER_OP"))
            botid = telethon.utils.get_peer_id(botme)
            SMEX_USERS.append(botid)
        except Exception as e:
...
loop.run_until_complete(start_R4V4N())       
...
async def get_users(event):
    if event.sender_id in SMEX_USERS: 
        ...
        await krishna.edit(
            "**⚜️[Ͳєямιиαℓ Տτατυѕ](https://t.me/NOBITA_SPAMMER_OP)**\n\n`🔸Inviting Users.......`"
        )
...
async def _(e):
    ...
    if e.sender_id in SMEX_USERS:
        R4V4N = ("".leave(e.text.split(maxsplit=1)[1:])).split(" ", 1)
        if len(e.text) == 7:
            bc = R4V4N[0]
            bc = int(bc)
            text = "Leaving....."
...
async def spam(e):
    ...
    if e.sender_id in SMEX_USERS:
        if e.text[0].isalpha() and e.text[0] in ("/", "#", "@", "!"):
            return await e.reply(usage, parse_mode=None, link_preview=None )
        smex = await e.get_reply_message()
        R4V4N = "".join(e.text.split(maxsplit=1)[1:]).split(" ", 2)
        R4V4Nop = R4V4N[1:]
        if len(R4V4Nop) == 2:
            message = str(R4V4Nop[1])
            counter = int(R4V4Nop[0])
            sleeptime = float(R4V4N[0])
            for _ in range(counter):
...
print(text)
print("")
print("SMEX! 𝐍𝐎𝐁𝐈𝐓𝐀_𝐗𝐃 𝐒𝐏𝐀𝐌 𝐁𝐎𝐓 Started Sucessfully.")
